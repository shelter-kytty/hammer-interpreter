...
static void binaryInt(Compiler* compiler, Token token) {
    long long value = strtoll(token.start + 2, NULL, 2);
    if (value <= UINT16_MAX) {
        emitShort(compiler, OP_INT_P, value, token.line);
    }
    else {
...
static void fString(Compiler* compiler, Token oldStr) {
    // consider refactoring this to use malloc from stdlib.h instead
    // avoids using the gc macros for an extremely temporary allocation
    char* newStr = ALLOCATE(compiler->vm, oldStr.length, char);

    size_t trueLen = 0;
    for (const char* ptr = oldStr.start + 2; ptr - oldStr.start < oldStr.length - 1; ptr++, trueLen++) {
        char tba = *ptr;

        if (tba == '\\') {
            switch (ptr[1]) {
            case '\\': ptr++; newStr[trueLen] = '\\'; break;
            case '"':  ptr++; newStr[trueLen] = '"';  break;
            case 'n':  ptr++; newStr[trueLen] = '\n'; break; 
            case 't':  ptr++; newStr[trueLen] = '\t'; break; 
    ...
    char* trueStr = ALLOCATE(compiler->vm, trueLen + 1, char);
    memcpy(trueStr, newStr, trueLen);
    trueStr[trueLen] = '\0';

    FREE_ARRAY(compiler->vm, newStr, oldStr.length, char);
...
static bool isSure(TokenType type) {
    switch (type) {
        ...
        case TOKEN_PERCENT:             return true; //    | Cannot produce a falsey value
        case TOKEN_UCARET:              return true; //    | Guaranteed to be truthy
...
static void variableIf(Compiler* compiler, TernaryExpr* ternary) {
...
static void openBlock(Compiler* compiler, BlockExpr* block)  {
    for (size_t i = 0; i < block->count - 1; ++i) {
        ...
        //printf("Just compiled expr %d in %s: ", i, getName(compiler));
